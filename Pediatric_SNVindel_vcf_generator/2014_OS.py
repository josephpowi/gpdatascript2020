...
for i in snvindelfh:
	l = i.replace('\n','').split('\t')
	SamNam = l[0]
	if SamNam in samAssay:
		dnaassay = samAssay[SamNam]
	else:
		print(i)
		continue
	chron = l[3].split('.')[0]
	...
	REF = l[6]
	ALT= l[7]
	out.write('\t'.join([SamNam,chron,pos,MinT,TinT,MinN,TinN,REF,ALT,dnaassay])+'\n')
snvindelfh.close()
out.close()
...
for i in fh:
	...
	if ID not in SQLITE:
		sqlitejs = SQLITEDB(SamNam,chron,POS,REF,ALT,l[9],l[3],l[4],l[5],l[6],'24703847','pcgp','somatic')
		SQLITE[ID] = sqlitejs
	else:
