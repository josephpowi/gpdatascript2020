#!/usr/bin/python3
usage = 'python3 '+__file__+'<IDSWI> <snvindel_supptable>'
...
#Sample match
IDswi = open(sys.argv[1])
IDSWI = {}
for ids in IDswi:
	idsL = ids.strip().split('\t')
	IDSWI[idsL[1]] = idsL[0]+'_D1'
IDswi.close()
...
for i in snvindelfh:
	l = i.split('\t')
	if l[2].startswith('SJEWS'):
		SamNam = l[2]+'_D1'
	elif l[2].startswith('IC'):
		if l[2] in IDSWI:
			SamNam = IDSWI[l[2]]
		else:
			ICNoFound.append(l[2])
			continue
	else:
		print(l[2])
		sys.exit(1)
	chron = l[3][3:] if l[3].startswith('chr') else l[3]
	pos = l[4].split('.')[0]
	MinT = l[9].split('.')[0]
	TinT = l[10].split('.')[0]
	MinN = l[11].split('.')[0]
...
for i in fh:
	...
	if ID not in SQLITE:
		sqlitejs = SQLITEDB(SamNam,chron,POS,REF,ALT,l[9],l[3],l[4],l[5],l[6],'25223734','pcgp','somatic')
		SQLITE[ID] = sqlitejs
	else:
